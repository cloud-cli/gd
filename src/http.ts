...
const sha1 = (secret: string, payload: string | Buffer) =>
...
export class HttpServer {
  ...
  checkProtectedRoute(request: HttpRequest, secret: string) {
    ...
    const payloadSignature =
      "sha1=" + sha1(secret, String(request.bodyText || request.body || ""));
    const valid = payloadSignature === requestSignature;

    if (!valid) {
      logger.error(
        `Invalid request ${request.id}, signature: ${requestSignature}, expected ${payloadSignature}`
      );
    }
  ...
  async dispatch(_request: IncomingMessage, _response: ServerResponse) {
    ...
    const match = this.routes.filter(
      (route) => route.method === method && route.match(String(url))
    );
  ...
  augmentResponse(response: HttpResponse) {
    ...
    response.send = function (
      status: Promise<any> | Error | number | Json | string,
      body = ""
    ) {
  ...
  fetch(url: string, requestOptions?: RequestOptions): Promise<FetchResponse> {
    return new Promise((resolve, reject) => {
      ...
      const request = (urlObject.protocol === "https:" ? getHttps : getHttp)(
        urlObject,
        requestOptions!
      );
  ...
  return new Promise((resolve) => {
    ...
    stream.on("end", () => {
      ...
      if (isJson) {
        ...
        stream.body = tryToParseJson(stream.bodyText);
      }
